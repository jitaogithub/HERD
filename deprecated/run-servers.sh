...
NUM_SERVERS=19			# Number of server processes on the server machine	
...
for i in `seq 1 $NUM_CLIENT_MACHINES`; do
	mc=`expr $i + 1`
	client_id=`expr $mc - 2`
	ssh -oStrictHostKeyChecking=no node-$mc.taoji-105332.wisr-PG0.utah.cloudlab.us "cd HERD; ./run-machine.sh $client_id" &
